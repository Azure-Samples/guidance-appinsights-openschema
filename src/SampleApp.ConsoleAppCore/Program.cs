...
namespace SampleApp.ConsoleAppCore
{
	using System;
	using System.IO;
	...
	{
		static ILogger _loggerDefault;
		static string _message = "SampleApp.ConsoleAppCore This is way too easy!";
		...
		public static void Main(string[] args)
		{
			...
			_loggerDefault = LoggingManager.ConfigureLogger(localLogger, Directory.GetCurrentDirectory(), logLevelCategory).GetAwaiter().GetResult();
			...
			for (int loop = 0; loop < iterationsLogs; loop++)
			{
				...
				_loggerDefault.LogMetric("TestMetricConsoleAppCore", 3);
				...
				logLevelCategory = "SampleAppCore";
				...
				{
					_loggerDefault.LogError(0, e, "MyTest");
					Console.WriteLine($"Exception {e.Source} {e.Message}");
				}
			}

			Console.WriteLine($"Delay some as logs are sent on a timer (or when buffer is full)...");
			Console.WriteLine($"{DateTime.Now:yyyy-MM-dd HH:mm:ss.fff} >> Completed {iterationsLogs} iterations");
			Console.WriteLine();
			Console.ReadKey();
		}
	}
